...
def get_list_teacher(sheet):
    ...
    for index, row in df['Phân bổ GD'].iterrows():
        ...
        list_teacher[teacher_id] = {
            'time_gl': round(time_all * 0.57, 2), # giờ giảng dạy trên lớp
            'time_gh': round(time_all * (1 - 0.57), 2), # giờ hướng dẫn đồ án
...
if __name__ == '__main__':
    pprint(get_list_teacher('NCM.xlsx'))
    # pprint(get_list_nguyen_vong('SV1.xlsx'))
    pprint(get_time_table('TKB.xlsx'))
