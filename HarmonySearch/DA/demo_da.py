...
def evaluate_solution(solution, teachers, aspirations):
    total_score = 0
    teacher_workload = {teacher_key: 0 for teacher_key in teachers.keys()}  # Tổng giờ làm việc
    teacher_assignments = {teacher_key: 0 for teacher_key in teachers.keys()}  # Số nguyện vọng được phân
...
def update_harmony(harmony_memory, teachers, aspirations, new_solution):
    ...
    worst_solution = min(harmony_memory, key=lambda sol: evaluate_solution(sol, teachers, aspirations))
    ...
    return harmony_memory
def harmony_search(teachers, aspirations, memory_size=HMS, max_iterations=MAX_ITER, hmcr=HMCR, par=PAR):
    harmony_memory = initialize_harmony_memory(teachers, aspirations, memory_size)
    teacher_workload = {teacher_key: 0 for teacher_key in teachers.keys()}  # Theo dõi tổng giờ đã phân công cho mỗi giáo viên
    teacher_assignments = {teacher_key: 0 for teacher_key in teachers.keys()}  # Theo dõi số nguyện vọng đã phân công

    for _ in range(max_iterations):
        new_solution = {}
        unassigned_aspirations = []  # Danh sách nguyện vọng chưa được phân công
        ...
        for aspiration_key, aspiration_data in aspirations.items():
            if aspiration_data["giang_vien"]:
                ...
                if (
                    teacher_workload[assigned_teacher] + aspiration_data["gio"] < 1.5 * teachers[assigned_teacher]["time_gh"]
                    ...
                    unassigned_aspirations.append(aspiration_key)  # Thêm vào danh sách chưa phân công
            else:
                if random.random() < hmcr:
                    ...
                    if (
                        selected_teacher
                        and teacher_workload[selected_teacher] + aspiration_data["gio"] < 1.5 * teachers[selected_teacher]["time_gh"]
                    ...
                    # Chọn ngẫu nhiên từ tập giáo viên khả dụng
                    valid_teachers = [
                        teacher_key
                        for teacher_key, teacher_data in teachers.items()
                        if teacher_workload[teacher_key] + aspiration_data["gio"] < 1.5 * teacher_data["time_gh"]
                        and teacher_assignments[teacher_key] < 30
                    ]
                    if valid_teachers:
                        ...
                        new_solution[aspiration_key] = None  # Không thể gán
                        unassigned_aspirations.append(aspiration_key)  # Thêm vào danh sách chưa phân công

                # Áp dụng Pitch Adjustment
                if random.random() < par and new_solution[aspiration_key]:
                    valid_teachers = [
                        teacher_key
                        for teacher_key, teacher_data in teachers.items()
                        if teacher_workload[teacher_key] + aspiration_data["gio"] < 1.5 * teacher_data["time_gh"]
      ...
      #   # Xử lý các nguyện vọng chưa được phân công
      #   for aspiration_key in unassigned_aspirations:
      ...
      #           teacher_assignments[selected_teacher] += 1
      #       else:
      #           new_solution[aspiration_key] = None  # Không thể phân công nếu không có giáo viên khả dụng

        harmony_memory = update_harmony(harmony_memory, teachers, aspirations, new_solution)

    best_solution = max(harmony_memory, key=lambda sol: evaluate_solution(sol, teachers, aspirations))
    ...
    while aspirations_no_teacher:
        ...
        for aspiration in aspirations_no_teacher:
            ...
            valid_teachers = [
                ...
                if teacher_key not in teacher_assignments or len(teacher_assignments[teacher_key]) < 30  # Điều kiện số nguyện vọng < 30
                and sum(item["hours_assigned"] for item in teacher_assignments.get(teacher_key, [])) + aspiration_data["gio"] <= 1.5 * teacher_data["time_gh"]  # Điều kiện giờ dạy
            ]
        ...
        aspirations_no_teacher = [asp for asp in aspirations_no_teacher if asp not in newly_assigned]
