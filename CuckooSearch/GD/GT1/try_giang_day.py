...
def evaluate(individual, teachers, classes):
    ...
    for i in range(individual.shape[0]):
        ...
        # Ràng buộc 5: Mỗi giảng viên chỉ dạy 1 lớp trong 1 thời điểm
    for i in range(individual.shape[0]):  # Với từng giảng viên
        ...
        time_gl = teachers[teacher_key]["time_gl"] 
        for class_key in assigned_classes:
            class_day = classes[class_key]["day"]
            class_period = set(classes[class_key]["period"])
            class_week = set(classes[class_key]["week"])
            ...
            if class_quy_doi_gio > 2 * time_gl:
                ...
                continue
            # Kiểm tra từng lớp đã được phân
            conflict = False
