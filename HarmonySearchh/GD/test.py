...
def harmony_search_teaching_assignment(teachers, classes):
    ...
    def generate_random_harmony():
        ...
        for class_id, class_info in classes.items():
            # Lọc các giảng viên đủ điều kiện dạy môn học của lớp
            eligible_teachers = [
                teacher_id
            ...
            if eligible_teachers:
                # Tính khối lượng công việc hiện tại của mỗi giảng viên đủ điều kiện
                teacher_workloads = {
                    teacher_id: calculate_workload(teacher_id, harmony)
                    for teacher_id in eligible_teachers
                }
                
                # Chọn giảng viên có ít khối lượng công việc nhất
                best_teacher = min(teacher_workloads, key=teacher_workloads.get)

                # Kiểm tra xem khối lượng công việc của giảng viên có vượt quá giới hạn hay không
                if calculate_workload(best_teacher, harmony) + class_info["quy_doi_gio"] <= 2 * teachers[best_teacher]["time_gl"]:
                    harmony[class_id] = best_teacher
                else:
                    # Nếu giảng viên có ít công việc nhất không thể dạy, chọn giảng viên khác trong danh sách đủ điều kiện
                    eligible_teachers.remove(best_teacher)
                    # Tính toán lại khối lượng công việc của các giảng viên còn lại
                    teacher_workloads = {
                        teacher_id: calculate_workload(teacher_id, harmony)
                        for teacher_id in eligible_teachers
                    }
...
if __name__ == "__main__":
    ...
    unassigned_classes = [
        ...
        if class_id not in best_solution
    ]
